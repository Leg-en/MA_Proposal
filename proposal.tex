...
\begin{linenumbers}
    ...
    \begin{questions}
        \item \label{rq:first_q} To what extent can a digital twin with an intuitive user interface enable non-expert users to accurately interpret and manipulate key wind energy parameters, as measured by user surveys and task completion rates?
    ...
    \end{questions}
    %Das klingt irgendiwe alles blöd, aber ich hab keine ahnung was ich hier schreiben sollte?! 
    The primary objective of \ref{rq:first_q} is to assess the interaction between non-expert users and a contemporary, intuitive digital twin. The evaluation will focus on the usability of the digital twin, with a specific emphasis on the ease with which users can interact with and comprehend the application. Consequently, the focus will be solely on the interaction with the digital twin.
    \ref{rq:second_q} aims to investigate the influence of the digital twin on public opinion concerning wind energy. In this context, influence refers to any measurable changes in attitude and support for wind energy initiatives resulting from the interaction with the digital twin.
    Both research questions will be addressed through a user study, which will be conducted as an integral component of the thesis.
